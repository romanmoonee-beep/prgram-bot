...
class CleanupWorker {
  ...
  private async processCleanup(job: Job<CleanupJob>) {
  ...
  private async cleanupOldData(olderThan?: Date) {
  ...
  private async cleanupDirectory(dirPath: string, maxAge: number, fileFilter?: RegExp): Promise<{ files: number, size: number }> {
  ...
  private async cleanupOldRedisData(cutoffDate: Date) {
...
export default CleanupWorker;
    ...
    // Также очищаем старые rate limit записи
    const rateLimitPattern = 'rl:*';
    cursor = '0';
    let deletedRateLimits = 0;
    
    do {
      const [newCursor, keys] = await redis.scan(cursor, 'MATCH', rateLimitPattern, 'COUNT', 100);
      cursor = newCursor;
      
      for (const key of keys) {
        const ttl = await redis.ttl(key);// src/jobs/workers/cleanupWorker.ts
import { Job } from 'bull';
import { cleanupQueue, CleanupJob } from '../queues';
import { Notification, Task, Check } from '../../database/models';
import { logger } from '../../utils/logger';
